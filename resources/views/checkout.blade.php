  ...
  border: 2px solid #f280b9ff;" > 
    ...
    <section class="header dark:bg-gray-900">
      <div class="grid max-w-screen-xl px-4 py-8 mx-auto lg:gap-8 xl:gap-0 lg:py-16 lg:grid-cols-12">
        <div class="hidden lg:flex lg:items-center lg:justify-end lg:col-span-5"></div>
      </div>
    </section> -->
  </div>
  <!---------------------------------------------->
  <div id="mainContent"  class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
    <div class="bg-imageLeft backimg"></div>
    <div class="bg-imageRight backimg"></div>
    <div id="contentChild">
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    buttons.forEach(button => {
      button.addEventListener('click', function () {
        const inputId = this.getAttribute('data-copy-to-clipboard-target');
        const input = document.getElementById(inputId);
        navigator.clipboard.writeText(input.value)
          .then(() => {
            const defaultMessage = this.querySelector('#default-message');
            const successMessage = this.querySelector('#success-message');
            defaultMessage.textContent = 'Copied';
            successMessage.style.display = 'inline-flex';
            setTimeout(function () {
              defaultMessage.textContent = 'Copy';
